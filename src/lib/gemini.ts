...
export async function generatePresentation(
  ...
  const timeout = (ms: number) => new Promise((_, reject) => 
    setTimeout(() => reject(new Error("Generation timed out. The AI took too long to respond. Please try again.")), ms)
  );

  const contentGeneration = async (): Promise<PresentationData> => {
  ...
  // Race between generation and 45s timeout
  return Promise.race([
    contentGeneration(),
    timeout(45000)
  ]) as Promise<PresentationData>;
}
...
export async function generateImage(prompt: string): Promise<string> {
  ...
  // Stable pollinations image endpoint
  // flux is state-of-the-art for visual quality
  const imageUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=1024&height=1024&seed=${uniqueSeed}&model=flux&nologo=true`;
